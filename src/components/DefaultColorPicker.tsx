...
const DefaultColorPicker = forwardRef(
  (props: ColorPickerProps, ref: ForwardedRef<HTMLDivElement>) => {
    ...
    const placeholder = props.placeholder
      ? props.placeholder
      : getLanguageStrings(lang).placeholder.replace(
        /%/g,
        format.toUpperCase(),
      );
    ...
    const [drag, setDragPrimitive] = useState<HTMLElement | undefined | null>(
      null,
    );
    ...
    const [controlPositions, setControlPositions] = useState<
      typeof initialControlPositions
    ...
    const pickerClass = () => {
      return `${open === pickerDropdown.current ? " " + position : ""}${
        pickerShown ? " show" : ""
      }`;
    };
    const menuClass = () => {
      return `${open === menuDropdown.current ? " " + position : ""}${
        menuShown ? " show" : ""
      }`;
    };
    ...
    const hidePicker = () => {
      ...
      emulateTransitionEnd(
        pickerDropdown.current as HTMLDivElement,
        hideTransitionEnd,
      );
    };
    ...
    function handleBlur(
      { relatedTarget, currentTarget }: FocusEvent<HTMLElement>,
    ) {
    ...
    const pointerUp = ({ target }: PointerEvent) => {
      ...
      // istanbul ignore else @preserve
      if (
        !dragRef.current && (!selection || !selection.toString().length) &&
