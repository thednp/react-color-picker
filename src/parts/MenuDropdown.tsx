...
import { type CSSProperties, type KeyboardEvent, forwardRef, ForwardedRef, useId } from 'react';
...
const PresetsMenu = (props: PresetsProps) => {
  const { locale, value, setColor, format, setValue, setInputValue, updateControlPositions } = usePickerContext();
...
const MenuDropdown = forwardRef((props: MenuProps, ref: ForwardedRef<HTMLDivElement>) => {
  const { colorKeywords, colorPresets, className, toggleMenu, pickerShown, menuShown } = props;
  ...
  const keyHandler = (e: KeyboardEvent<HTMLElement>) => {
    ...
    if (isColorOptionsMenu) {
      ...
      if (previousElement && key === keyArrowUp) {
        ...
        focus(nextElement);
      } else if (typeof previousElementSibling !== 'undefined' && key === keyArrowLeft) {
        focus(previousElementSibling as HTMLElement);
      } else if (typeof nextElementSibling !== 'undefined' && key === keyArrowRight) {
        focus(nextElementSibling as HTMLElement);
      }
    } else if (typeof previousElementSibling !== 'undefined' && [keyArrowLeft, keyArrowUp].includes(key)) {
      focus(previousElementSibling as HTMLElement);
    } else if (typeof nextElementSibling !== 'undefined' && [keyArrowRight, keyArrowDown].includes(key)) {
      focus(nextElementSibling as HTMLElement);
    }
